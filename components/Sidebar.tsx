...
const Sidebar: React.FC<SidebarProps> = ({ user, activePage }) => {
  const menuItems = [
    ...
    { id: 'students', label: 'Alunos', icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 01-9-3.47m0 0a6 6 0 0112 0v1z" />
      </svg>
    )},
    { id: 'courses', label: 'Gestão de Cursos', icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <aside className="w-64 bg-white border-r border-slate-100 flex flex-col h-screen sticky top-0 left-0 overflow-y-auto">
      <div className="p-6 border-b border-slate-50">
        <div className="flex items-center gap-3">
          ...
          <span className="text-lg font-bold text-slate-900">EduTech <span className="text-blue-600">Admin</span></span>
        </div>
      </div>
